...
// open from either the name *or* the cashtag
['.profile-name', '.cashtag'].forEach(sel=>{
    document.querySelector(sel).addEventListener('click', () => showSheet('accountSheet'));
});

// close by tapping the dark backdrop
document.querySelector('#accountSheet .sheet-backdrop')
        .addEventListener('click', () => hideSheet('accountSheet'));

// “create business account” demo
document.getElementById('addBusinessAcct').addEventListener('click', ()=>{
    hideSheet('accountSheet');
    showMessageBox('Business Account','Simulating business-account signup (demo only).');
});
