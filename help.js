...
// =====================================================================================================================
let balance = 1500.51; // Current cash balance. This will fluctuate with simulated transactions. (Starting higher for K-format demo)
 ...
 * applying 'K' for thousands and handling signs.
...
function formatAmountForDisplay(amount, includeCents = true) {
    ...
    if (absAmount >= 1000) {
        ...
        // Round to nearest whole number for K display
...
function updateAmount(value) {
    ...
    // Re-parse after capping
    const finalNumericAmount = parseFloat(amountValue);

    // Format the displayed amount for currency (e.g., "$12.34")
    if (isNaN(finalNumericAmount)) {
...
function showDetailScreen(screenType) {
    ...
    if (screenType === 'cashBalanceDetail') {
        ...
        document.getElementById('copyRoutingNumberButton').addEventListener('click', () => {
            document.execCommand('copy'); // Simulate copy
